...
const Products = () => {
  ...
  const fetchData = async () => {
    try {
      const [productsResponse, categoriesResponse] = await Promise.all([supabase.from("products").select("*").eq("is_active", true), supabase.from("categories").select("*").eq("is_active", true)]);
  ...
  const generateSearchSuggestions = () => {
    ...
    products.forEach(product => {
      const searchableTerms = [product.name, product.description || '', product.product_type || '', product.origin || ''];
      ...
      <main className="pb-20 md:pb-0">
        ...
        <div className="bg-card rounded-lg shadow-sm border p-4 mb-8">
                  ...
                  <DrawerContent>
                    ...
                    <div className="p-4 space-y-4">
                      ...
                      <div className="flex gap-3 pt-4">
                        ...
                        {(searchTerm || selectedCategory !== "all" || sortBy !== "name") && <Button variant="outline" onClick={clearFilters} className="flex-1">
                            Effacer
                          </Button>}
                      </div>
                    </div>
              ...
              <div className="relative">
                ...
                <Input placeholder="Rechercher un produit..." value={searchTerm} onChange={e => {
              setSearchTerm(e.target.value);
              setShowSuggestions(true);
            }} onFocus={() => setShowSuggestions(true)} onBlur={() => setTimeout(() => setShowSuggestions(false), 200)} className="pl-10 transition-all duration-200 focus:ring-2 focus:ring-primary/20" autoComplete="off" />
              ...
              setShowSuggestions(false);
                ...
                {/* Search Suggestions */}
                {showSuggestions && searchSuggestions.length > 0 && <div className="absolute top-full left-0 right-0 bg-popover border border-border rounded-lg shadow-lg mt-1 z-50 max-h-48 overflow-y-auto">
                    {searchSuggestions.map((suggestion, index) => <button key={index} onClick={() => handleSearchSuggestionClick(suggestion)} className="w-full px-4 py-2 text-left hover:bg-accent transition-colors flex items-center gap-2">
              ...
              {/* Active Filters */}
              {(searchTerm || selectedCategory !== "all") && <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t">
        ...
        <div className="bg-card rounded-lg shadow-sm border p-6 mb-8">
              <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* Clear Filters */}
                  {(searchTerm || selectedCategory !== "all" || sortBy !== "name") && <Button variant="outline" onClick={clearFilters}>
                      Effacer les filtres
                    </Button>}
                </div>
              </div>
          ...
          {/* Products Grid/List */}
          {filteredProducts.length > 0 ? <div className={`${viewMode === "grid" ? "grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6" : "space-y-4"} animate-fade-in`}>
              {filteredProducts.map((product, index) => <div key={product.id} className="animate-fade-in" style={{
            animationDelay: `${index * 50}ms`
          }}>
                  <ProductCard id={product.id} name={product.name} description={product.description || ""} price={product.base_price} image={product.image_url || "/placeholder.svg"} unitType={product.unit_type as 'kg' | 'units' | 'g' | 'pieces'} inStock={product.stock_quantity ? product.stock_quantity > 0 : true} featured={product.featured || false} />
                </div>)}
