  ...
  return <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="left" className="w-[300px] sm:w-[400px] p-0">
        ...
        <div className="p-6 space-y-4 overflow-y-auto max-h-[calc(100vh-200px)]">
          ...
          return <button key={category.id} onClick={() => handleCategoryClick(category.slug)} className="w-full p-4 rounded-lg border border-gray-200 hover:border-ocean hover:shadow-md transition-all duration-200 text-left group">
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-ocean/10 rounded-lg group-hover:bg-ocean/20 transition-colors">
                      {category.custom_svg ? <div className="h-6 w-6 text-ocean" dangerouslySetInnerHTML={{
                  __html: category.custom_svg
                }} /> : IconComponent ? <IconComponent className="h-6 w-6 text-ocean" /> : <Fish className="h-6 w-6 text-ocean" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="font-semibold transition-colors text-slate-50">
                          {category.name}
                        </h3>
                      </div>
